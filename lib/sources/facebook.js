...
//
//  If more than one parser results in an attachment, then all are listed.
...
FacebookSource.prototype.fetch = function (date) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        return Promise.map(posts, function(p) {
            var promises = chains.map(function(c) {
                var context = {
                    date: new Date((date || new Date()).getTime())
                };

                if (c.filter && !c.filter(p, context)) {
                    return new Promise.reject();;
                }

                return c.parser.bind(that)(p, context);
            });
